  ...
  def find_match
    ...
    sql = "select * from trips t join pools p on p.trip_id = t.id join users u on p.user_id = u.id
        where p.is_active = true and u.id != :uid and
            ...
            ST_DWithin( ST_geometryN(t.waypoints, 1)::geography , :origin::geography, 5000 ) and
            ST_DWithin( ST_geometryN(t.waypoints, 2)::geography , :destination::geography, 1000 )
            limit 20"
    vars = {uid: @current_user.id, driving_pref: @current_user[:driving_pref], to_work: @current_trip[:to_work],
    ...
    # - the match trip's origin is within 5km of the current trip's origin (note the difference in indexing!)
    # - the match trip's destination is within 1km of the current trip's destination
    # SHOULD HAVE SOME WAY OF RELAXING IF NOT ENOUGH RESULTS / BEING MORE PICKY IF TOO MANY. ALSO TIMES NOT CONSIDERED
